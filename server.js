...
app.use(
  cors({
    origin: [
      ...
      "http://localhost:3001",
      "http://127.0.0.1:3001",
      "https://becsserver-8ws3ghone-tathagatas-projects.vercel.app",
...
app.post("/complete-profile", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ status: "error", message: "Update failed due to server error." });
  }
});
...
app.get(
  ...
  async (req, res, next) => {
    ...
    if (uid !== req.user.uid) {
      console.warn(
        `❌ UID mismatch: requested(${uid}) vs authenticated(${req.user.uid})`
      );
      return res.status(403).json({
        error: "Forbidden",
...
app.post(
  ...
  async (req, res, next) => {
    ...
    if (
      ...
      console.warn(
        `[${new Date().toISOString()}] ❌ Missing required fields: course_id=${course_id}, payment_id=${razorpay_payment_id}, order_id=${razorpay_order_id}, signature=${razorpay_signature}`
      );
      return res.status(400).json({
        error: "Bad Request",
        message:
          "Course ID, razorpay_payment_id, razorpay_order_id, and razorpay_signature are required",
        timestamp: new Date().toISOString(),
      });
    }
    try {
      ...
      if (generatedSignature !== razorpay_signature) {
        console.warn(
          `[${new Date().toISOString()}] ❌ Payment verification failed for order: ${razorpay_order_id}`
        );
        return res.status(400).json({
          status: "error",
          message: "Invalid payment signature",
      ...
      if (existingPurchase.length > 0) {
        console.warn(
          `[${new Date().toISOString()}] ❌ Course ${course_id} already purchased by user ${uid}`
        );
        return res.status(400).json({
          error: "Bad Request",
      ...
      console.error(
        `[${new Date().toISOString()}] ❌ Error processing course purchase:`,
        err
      );
      if (err.code === "ER_NO_REFERENCED_ROW_2") {
        console.warn(
          `[${new Date().toISOString()}] ❌ Invalid course ID: ${course_id}`
        );
        return res.status(400).json({
          status: "error",
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        if (purchaseExists.length === 0) {
          console.warn(
            `❌ User ${req.user.uid} has not purchased course ${courseId}`
          );
          return res.status(403).json({
            status: "error",
        ...
        console.error(
          `❌ Attempt ${attempt} failed: Error fetching lectures for course ${courseId}:`,
          err
        );
        if (
          (err.code === "ECONNRESET" ||
